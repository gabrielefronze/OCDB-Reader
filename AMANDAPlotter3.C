...
void AMANDAPlotter3(TString inputFileName){
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            ...
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                ...
                for(Int_t iEntry=0;iEntry<nEntries-1;iEntry++){
                    ...
                    if((followingTimestamp-epochStart)%rebinEntity<10){
            ...
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                if(RPC!=maxChargeRPC[plane][side]){
                    if(!(plane==3 && side==0 && RPC==3)){
                        if(!(plane==1 && side==1 && RPC==2)){
                            delete graphCharge[side][plane][RPC-1];
                            delete graphTot[side][plane][RPC-1];
                        }
                    }
                }
            }

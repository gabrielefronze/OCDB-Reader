...
void AMANDAPlotter4(TString inputFileName){
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                ...
                for(Int_t iEntry=0;iEntry<nEntries-1;iEntry++){
            		...
            		charge[side][plane][RPC-1]+=current->GetINet()*(Double_t)(followingTimestamp-actualTimestamp);
            		actualTimestamp=current->GetTimeStamp();
    ...
    for (Int_t side=0; side<nSides; side++) {
        for (Int_t plane=0; plane<nPlanes; plane++) {
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                printf("%s %d %d %f\n",sides[side],planes[plane],RPC,charge[side][plane][RPC-1]);
            }
        }
    }
}
...
void OCDBAnalyzerWithRates2(TString inputFileName){
    ...
    canvRates[1]=new TCanvas(Form("canv_%s_rates_not_bend",sides[0].Data()),Form("canv_%s_rates_not_bend",sides[0].Data()));
    canvRates[2]=new TCanvas(Form("canv_%s_rates_bend",sides[1].Data()),Form("canv_%s_rates_bend",sides[1].Data()));
        ...
        for (Int_t side=0; side<nSides; side++) {
            for (Int_t plane=0; plane<nPlanes; plane++) {
                for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                    ...
                    graphRates[cathode][side][plane][RPC-1]->SetTitle(Form("Rates_MT%d_%s_%s",planes[plane],sides[side].Data(),cathodes[cathode].Data()));
                    ...
                    for(Int_t iList=0; iList<sortedListScalers->GetEntries(); iList++){
                        ...
                        if (currentRun!=scalerData->GetRunNumber()){
                            ...
                            if (elapsedTime!=0.){
                                ...
                                graphRates[cathode][side][plane][RPC-1]->SetPoint(runCount, currentRun, currentRunAverageRate*2.);
                                runCount++;

                                Double_t averageCurrent=0.;
                                Int_t currentMeasuresCount=0;
                                AliRPCValueDCS *value;
                                for (Int_t iList2=0; iList2<sortedList->GetEntries(); iList2++){
                                    ...
                                    if(value->GetRunNumber()!=currentRun){
                                        ...
                                        if(value->IsCurrent()){
                                            ...
                                            if(!(valueCurrent->GetINet()<0.))averageCurrent+=valueCurrent->GetINet();
                                ...
                                if(currentMeasuresCount!=0){
                                    ...
                                    if(averageCurrent<0){
                                        cout<<"less than 0 net current in run "<<currentRun<<endl;
                                    } else {

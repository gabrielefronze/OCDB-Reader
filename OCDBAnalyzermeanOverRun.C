...
void OCDBAnalyzermeanOverRun(TString inputFileName){
    ...
    for (Int_t side=0; side<nSides; side++) {
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                ...
                //loop sulle entries
                Bool_t onlyOnce=kTRUE;
                Int_t previousRunNumber=0;
                Int_t currentRunNumber=0;
                Double_t runMeanDarkCurrent=0.;
                Int_t runMeanDarkCurrentCount=0;
                Int_t runMeanDarkCurrentPointCount=0;
                Double_t runMeanTotalCurrent=0.;
                Int_t runMeanTotalCurrentCount=0;
                Int_t runMeanTotalCurrentPointCount=0;
                Double_t runMeanNetCurrent=0.;
                Int_t runMeanNetCurrentCount=0;
                Int_t runMeanNetCurrentPointCount=0;
                Double_t runMeanVoltage=0.;
                Int_t runMeanVoltageCount=0;
                Int_t runMeanVoltagePointCount=0;
                ULong64_t runTimestamp=0;
                ...
                for(Int_t iList=0; iList<sortedList->GetEntries(); iList++){
                    ...
                    if(onlyOnce){
                        previousRunNumber=valueDCS3->GetRunNumber();
                        onlyOnce=kFALSE;
                    }
                    currentRunNumber=valueDCS3->GetRunNumber();
                    runTimestamp=valueDCS3->GetTimeStamp();
                    ...
                    if(currentRunNumber==previousRunNumber){
                        ...
                        if (valueDCS3->IsVoltage()){
                            ...
                            if (voltageBuffer->GetVSupp()>=8500.) {
                                ...
                                runMeanVoltageCount++;
                                runMeanVoltage+=voltageBuffer->GetVSupp();
                            ...
                            if (currentBuffer->GetIsCalib()==kFALSE) {
                                ...
                                runMeanDarkCurrent+=(Double_t)currentBuffer->GetIDark();
                                runMeanDarkCurrentCount++;
                                runMeanTotalCurrent+=(Double_t)currentBuffer->GetITot();
                                runMeanTotalCurrentCount++;
                                runMeanNetCurrent+=(Double_t)currentBuffer->GetITot()-(Double_t)currentBuffer->GetIDark();
                                runMeanNetCurrentCount++;                   
                            }
                        } else continue;
                    } else {
                        if(runMeanTotalCurrentCount!=0 && runMeanTotalCurrent!=0.){
                            graphTot[side][plane][RPC-1]->SetPoint(runMeanTotalCurrentPointCount, (Double_t)runTimestamp, runMeanTotalCurrent/(Double_t)runMeanTotalCurrentCount/Areas[RPC-1][plane]);
                            cout<<runMeanTotalCurrentPointCount<<endl;
                            runMeanTotalCurrentPointCount++;
                        }
                        if(runMeanDarkCurrentCount!=0 && runMeanDarkCurrent!=0.){
                            graphDarkCalib[side][plane][RPC-1]->SetPoint(runMeanDarkCurrentPointCount, (Double_t)runTimestamp, runMeanDarkCurrent/(Double_t)runMeanDarkCurrentCount/Areas[RPC-1][plane);
                            cout<<runMeanDarkCurrentPointCount<<endl;
                            runMeanDarkCurrentPointCount++;
                        }
                        if(runMeanNetCurrentCount!=0 && runMeanNetCurrent!=0.){
                            graphNet[side][plane][RPC-1]->SetPoint(runMeanNetCurrentPointCount, (Double_t)runTimestamp, runMeanNetCurrent/(Double_t)runMeanNetCurrentCount/Areas[RPC-1][plane);
                            cout<<runMeanNetCurrentPointCount<<endl;
                            runMeanNetCurrentPointCount++;
                        }
                        if(runMeanVoltageCount!=0 && runMeanVoltage!=0.){
                            graphV[side][plane][RPC-1]->SetPoint(runMeanVoltagePointCount, (Double_t)runTimestamp, runMeanVoltage/(Double_t)runMeanVoltageCount);
                            cout<<runMeanVoltagePointCount<<endl;
                            runMeanVoltagePointCount++;
                        }
                        //printf("Added point run:%d timestamp:%lu\n",currentRunNumber,runTimestamp);
                        runTimestamp=0;

                        previousRunNumber=currentRunNumber;
                        iList--;
                        currentRunNumber=0;
                        runMeanDarkCurrent=0.;
                        runMeanDarkCurrentCount=0;
                        runMeanTotalCurrent=0.;
                        runMeanTotalCurrentCount=0;
                        runMeanNetCurrent=0.;
                        runMeanNetCurrentCount=0;
                        runMeanVoltage=0.;
                        runMeanVoltageCount=0;
                    }    
                }
                ...
                for (Int_t iNetGraph=0; iNetGraph<graphNet[side][plane][RPC-1]->GetN()-1; iNetGraph++) {
                    ...
                    printf("start inet:%f timestamp:%lu\n",netCurrentStart,timeStampStart);
                    printf("stop inet:%f timestamp:%lu\n",netCurrentStop,timeStampStop);
    ...
    TGraph *graphMeanRateCurrentCorrelation;
    graphMeanRateCurrentCorrelation=new TGraph();
    graphMeanRateCurrentCorrelation->SetLineColor(color[RPC-1]);
    graphMeanRateCurrentCorrelation->SetMarkerColor(color[RPC-1]);
    graphMeanRateCurrentCorrelation->SetMarkerStyle(20);
    graphMeanRateCurrentCorrelation->SetMarkerSize(0.20);
    graphMeanRateCurrentCorrelation->SetTitle(Form("mean_rate_mean_current_correlation_MT%d_%s_%d",planes[plane],sides[side].Data(),RPC));
    TCanvas *canvMeanRateCurrentCorrelation;
    canvMeanRateCurrentCorrelation=new TCanvas(Form("canv_MT%d_%s_RPC_%d_mean_rate_mean_current_correlation",planes[plane],sides[side].Data(),RPC),Form("canv_MT_%d_%s_RPC_%d_mean_rate_mean_current_correlation",planes[plane],sides[side].Data(),RPC));
            ...
            for (Int_t plane=0; plane<nPlanes; plane++) {
                for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                            ...
                            if (elapsedTime!=0.){
                                ...
                                if(currentMeasuresCount!=0){
                                    ...
                                    if(averageCurrent<0){
                                        ...
                                        graphChargePerHit[cathode][side][plane][RPC-1]->SetPoint(runCount2, startTime, averageCurrent/(currentRunAverageRate*2.));
                                        if (cathode==1){
                                            graphRateCurrentCorrelation[side][plane][RPC-1]->SetPoint(runCount2, currentRunAverageRate*2./Areas[RPC-1][plane], averageCurrent/Areas[RPC-1][plane]);
                                            //cout<<"setting:\t"<<RPC<<"\t"<<currentRun<<"\t"<<currentRunAverageRate<<"\t"<<averageCurrent<<endl;
                                            graphMeanRPCIDark[side][plane][RPC-1]->SetPoint(runCount2, startTime, averageCurrent);
                                            graphMeanRPCRate[side][plane][RPC-1]->SetPoint(runCount2, startTime, currentRunAverageRate);
                                        }
                                        runCount2++;   
                                    }
                                }
                    ...
                    if (RPC==1) {
                        ...
                        canvRatesPerCm2[index]->cd(plane+1);
                        graphRatesPerCm2[cathode][side][plane][RPC-1]->GetYaxis()->SetRangeUser(0.,160000.);
                        graphRatesPerCm2[cathode][side][plane][RPC-1]->Draw("lp");
                        canvChargePerHit[index]->cd(plane+1);
                        graphChargePerHit[cathode][side][plane][RPC-1]->GetYaxis()->SetRangeUser(-0.0003,0.0014);
                        graphChargePerHit[cathode][side][plane][RPC-1]->Draw("lp");
                    }
    ...
    for (Int_t side=0; side<nSides; side++) {
        for (Int_t plane=0; plane<nPlanes; plane++) {             
            ...
            graphMeanIDark[side][plane]->SetLineColor(kPink);
            graphMeanIDark[side][plane]->SetMarkerColor(kPink);
            ...
            graphMeanRate[side][plane]->SetLineColor(kPink);
            graphMeanRate[side][plane]->SetMarkerColor(kPink);
    ...
    //cout<<"CIAO"<<endl;
            ...
            graphMeanIDark[side][plane]->Draw("alp");
            canvMeanValues[side][plane]->cd(2);
            graphMeanRate[side][plane]->Draw("alp");

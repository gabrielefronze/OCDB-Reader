...
void OCDBParserWithScalers2(Bool_t file=kFALSE, TString filename=""){
    ...
    TSortedList *scalersDataList[2][nSides][nPlanes][nRPC];
            ...
            for (Int_t plane=0; plane<nPlanes; plane++) {
                for (Int_t cathode=0; cathode<nCathodes; cathode++) {
                    for (Int_t localBoard=1; localBoard<=nLocalBoards; localBoard++) {
                        ...
                        if(scalerEntry!=arrayScalersEntries-1)scalersDataList[cathode][iSide][plane][iRPC09-1]->Add(new AliRPCValueScaler(*runIterator, SOR+elapsedTime, scalersData->GetLocScalStrip(cathode, plane, localBoard), isCalib));
                        // altrimenti il timestamp è pari all'EOR
                        else scalersDataList[cathode][iSide][plane][iRPC09-1]->Add(new AliRPCValueScaler(*runIterator, EOR, scalersData->GetLocScalStrip(cathode, plane, localBoard), isCalib));

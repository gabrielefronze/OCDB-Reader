...
void OCDBParserWithScalers4(Bool_t file=kFALSE, TString filename="",Int_t analyzedYear){
                ...
                for (Int_t cathode=0; cathode<nCathodes; cathode++) {
                    ...
                    for (Int_t localBoard=0; localBoard<nLocalBoards; localBoard++) {
                        ...
                        if(scalerEntry!=arrayScalersEntries-1){
                            scalersDataList[cathode][iSide][plane][iRPC09-1]->Add(new AliRPCValueScaler(*runIterator, SOR+elapsedTime, scalersData->GetLocScalStrip(cathode, plane, localBoard), isCalib, scalersData->GetDeltaT(), isOverflow));
                            scalersLocalBoardList[cathode][plane][localBoard]->Add(new AliRPCValueScaler(*runIterator, SOR+elapsedTime, scalersData->GetLocScalStrip(cathode, plane, localBoard), isCalib, scalersData->GetDeltaT(), isOverflow));
                        }
                        // altrimenti il timestamp è pari all'EOR
                        else {
                            scalersDataList[cathode][iSide][plane][iRPC09-1]->Add(new AliRPCValueScaler(*runIterator, EOR, scalersData->GetLocScalStrip(cathode, plane, localBoard), isCalib, scalersData->GetDeltaT(), isOverflow));
                            scalersLocalBoardList[cathode][plane][localBoard]->Add(new AliRPCValueScaler(*runIterator, EOR, scalersData->GetLocScalStrip(cathode, plane, localBoard), isCalib, scalersData->GetDeltaT(), isOverflow));
                        }

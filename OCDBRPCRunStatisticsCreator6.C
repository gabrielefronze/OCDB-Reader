...
void OCDBRPCRunStatisticsCreator6(TString inputFileName, Bool_t isDark){
	...
	THashList *sortedListData[nSides][nPlanes][nRPC];
	THashList *sortedListScalers[nSides][nPlanes][nRPC][nCathodes];
    THashList *scalersLocalBoardList[nCathodes][nPlanes][nLocalBoards];
    ...
    for (Int_t side=0; side<nSides; side++) {
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            ...
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                ...
                buffer->Sort();
                sortedListData[side][plane][RPC-1]=(THashList*)buffer;
                buffer=0x0;
            	...
            	for(Int_t cathode=0;cathode<nCathodes;cathode++){
                    ...
                    buffer->Sort();
                    sortedListScalers[side][plane][RPC-1][cathode]=(THashList*)buffer;
                    buffer=0x0;
    ...
    for(Int_t cathode=0;cathode<nCathodes;cathode++){
        ...
        for(Int_t plane=0;plane<nPlanes;plane++){
            ...
            for(Int_t local=0;local<nLocalBoards;local++){
                ...
                buffer->Sort();
                scalersLocalBoardList[cathode][plane][local]=(THashList*)buffer;
                buffer=0x0;
		                        ...
		                        if(iSide == side && iRPC09==RPC){
		                        	...
		                        	for(Int_t iScaler=previousScalers[cathode][plane][localBoard-1];iScaler<scalersLocalBoardList[cathode][plane][localBoard-1]->GetEntries();iScaler++){
                                        ...
                                        if(valueScaler->GetRunNumber()<previousRunNumber){
                                            ...
                                            //cout<<"continue "<<iScaler<<" "<<valueScaler->GetRunNumber()<<"<"<<previousRunNumber<<endl;
		                        			continue;
		                        		} else if(valueScaler->GetRunNumber()>previousRunNumber || iScaler==scalersLocalBoardList[cathode][plane][localBoard-1]->GetEntries()-1) {
                                            //cout<<"breaking"<<iScaler<<" "<<valueScaler->GetRunNumber()<<">"<<previousRunNumber<<endl;
                                            previousScalers[cathode][plane][localBoard-1]=iScaler;
		                        			break;
		                        		} else{

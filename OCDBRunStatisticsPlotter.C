...
void OCDBRunStatisticsPlotter(TString inputFileName){
	...
	TGraph *darkCurrentVariationVsCharge= new TGraph();
    darkCurrentVariationVsCharge->SetName("Dark current variations vs integrated charge");
    darkCurrentVariationVsCharge->SetTitle("Dark current variations vs integrated charge");
	darkCurrentVariationVsCharge->SetLineColor(kPink);
    darkCurrentVariationVsCharge->SetMarkerColor(kPink);
    darkCurrentVariationVsCharge->SetMarkerStyle(20);
    darkCurrentVariationVsCharge->SetMarkerSize(0.27);
    TGraph *darkRateVariationVsCharge= new TGraph();
    darkRateVariationVsCharge->SetName("Dark rate variations vs integrated charge");
    darkRateVariationVsCharge->SetTitle("Dark rate variations vs integrated charge");
    darkRateVariationVsCharge->SetLineColor(kPink);
    darkRateVariationVsCharge->SetMarkerColor(kPink);
    darkRateVariationVsCharge->SetMarkerStyle(20);
    darkRateVariationVsCharge->SetMarkerSize(0.27);
	TGraph *darkCurrentVariationVsDarkRateVariation= new TGraph();
    darkCurrentVariationVsDarkRateVariation->SetName("Dark current variations vs Dark rate variation");
    darkCurrentVariationVsDarkRateVariation->SetTitle("Dark current variations vs Dark rate variation");
	darkCurrentVariationVsDarkRateVariation->SetLineColor(kPink);
    darkCurrentVariationVsDarkRateVariation->SetMarkerColor(kPink);
    darkCurrentVariationVsDarkRateVariation->SetMarkerStyle(20);
    darkCurrentVariationVsDarkRateVariation->SetMarkerSize(0.27);
    Int_t deltaCount=0;
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
            	...
            	for(Int_t iList=0;iList<sortedListRunStatistics[side][plane][RPC-1]->GetEntries();iList++){
                    ...
                    //cout<<runStats->GetTimeStampStop()<<"\t\t"<<runStats->GetMeanNetCurrent()<<"\t"<<integratedChargeValue<<"\t"<<runStats->GetRunNumber()<<endl;
            		...
            		if(iList==firstNotCalib && controlStartingValues<=startingValues){
            			...
            			darkCurrentVariationVsCharge->SetPoint(deltaCount, integratedChargeValue/Areas[RPC-1][plane], deltaDarkCurrents[side][plane][RPC-1]/beginningDarkCurrents[side][plane][RPC-1]*100.);
                        darkRateVariationVsCharge->SetPoint(deltaCount, integratedChargeValue/Areas[RPC-1][plane], deltaDarkRates[side][plane][RPC-1]/beginningDarkRates[side][plane][RPC-1]*100.);
            			darkCurrentVariationVsDarkRateVariation->SetPoint(deltaCount, deltaDarkRates[side][plane][RPC-1]/beginningDarkRates[side][plane][RPC-1]*100., deltaDarkCurrents[side][plane][RPC-1]/beginningDarkCurrents[side][plane][RPC-1]*100.);
                        deltaCount++;
            		}
    ...
    canvVariations->cd();
    darkCurrentVariationVsCharge->GetXaxis()->SetTitle("Integrated charge [#muC/cm^{2}]");
    darkCurrentVariationVsCharge->GetYaxis()->SetTitle("#frac{#Deltai_{Dark}}{i_{Dark_{0}}} [%]");
    darkCurrentVariationVsCharge->Draw("ap");
    //canvVariations->cd(2);

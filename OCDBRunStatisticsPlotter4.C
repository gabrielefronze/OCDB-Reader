...
void OCDBRunStatisticsPlotter4(TString inputFileNamePhysRuns, TString inputFileNameCalibRuns){
    ...
    ppMeanAverageCurrentVsRate->SetName("pp mean average current vs rate");
    ppMeanAverageCurrentVsRate->SetTitle("pp_mean_average_current_vs_rate");
    ...
    PbPbMeanAverageCurrentVsRate->SetName("PbPb mean average current vs_rate");
    PbPbMeanAverageCurrentVsRate->SetTitle("PbPb_mean_average_current_vs_rate");
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            ...
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
            	...
            	for(Int_t iList=0;iList<sortedListRunStatistics[side][plane][RPC-1]->GetEntries();iList++){
                    ...
                    if(runStats->GetMeanRateNotBending()!=0.){
                        if(runStats->GetRunNumber()<firstPbPbRun) ppAverageCurrentVsRate[side][plane][RPC-1]->SetPoint(ppCount++, runStats->GetMeanRateNotBending()/Areas[RPC-1][plane], runStats->GetMeanNetCurrent()/Areas[RPC-1][plane]);
                        else PbPbAverageCurrentVsRate[side][plane][RPC-1]->SetPoint(ppCount++, runStats->GetMeanRateNotBending()/Areas[RPC-1][plane], runStats->GetMeanNetCurrent()/Areas[RPC-1][plane]);
                ...
                for(Int_t iList2=sortedListRunStatistics[side][plane][RPC-1]->GetEntries()-1;iList2>0;--iList2){
                    ...
                    if(controlEndingValuesCurrent<=valuesCurrent || controlEndingValuesRate<=valuesRate){
                        //cout<<"setting beginning"<<endl;
                        if(runStats2->GetMeanDarkCurrent()!=0.){
                            deltaDarkCurrents[side][plane][RPC-1]+=runStats2->GetMeanDarkCurrent();
    ...
    for(std::vector<Int_t>::iterator runNumberIt = runNumbers.begin(); runNumberIt != runNumbers.end(); ++runNumberIt){
        if(*runNumberIt<firstPbPbRun) ppMeanAverageCurrentVsRate->SetPoint(count1++, RPCData->GetMeanRateNotBending(*runNumberIt), RPCData->GetMeanNetCurrent(*runNumberIt));
        else PbPbMeanAverageCurrentVsRate->SetPoint(count2++, RPCData->GetMeanRateNotBending(*runNumberIt), RPCData->GetMeanNetCurrent(*runNumberIt));
        ...
        darkRateVsTime->SetPoint(count, RPCData->GetMeanTimeStampStart(*runNumberIt), RPCData->GetMeanRateNotBending(*runNumberIt));
        darkCurrentVsTime->SetPoint(count, RPCData->GetMeanTimeStampStart(*runNumberIt), RPCData->GetMeanDarkCurrent(*runNumberIt));
        count++;
    }
    ...
    darkCurrentVariationVsDarkRateVariationMulti->GetYaxis()->SetTitle("Integrated charge [#muC/cm^{2}]");

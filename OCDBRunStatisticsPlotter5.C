...
void OCDBRunStatisticsPlotter5(TString inputFileNamePhysRuns, TString inputFileNameCalibRuns){
    ...
    for(std::vector<Int_t>::iterator runNumberIt = runNumbers.begin(); runNumberIt != runNumbers.end(); ++runNumberIt){
        if(*runNumberIt<firstPbPbRun){
            ppMeanAverageCurrentVsRate->SetPoint(count1++, RPCData->GetMeanRateNotBending(*runNumberIt), RPCData->GetMeanNetCurrent(*runNumberIt));
            ppMeanAverageDarkCurrentVsHV->SetPoint(count1++, RPCData->GetMeanHV(*runNumberIt), RPCData->GetMeanDarkCurrent(*runNumberIt));
        }
        else {
            PbPbMeanAverageCurrentVsRate->SetPoint(count2++, RPCData->GetMeanRateNotBending(*runNumberIt), RPCData->GetMeanNetCurrent(*runNumberIt));

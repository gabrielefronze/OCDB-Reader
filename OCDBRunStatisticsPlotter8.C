...
void OCDBRunStatisticsPlotter8(TString inputFileNamePhysRuns, TString inputFileNameCalibRuns, TString AMANDAChargesFile, TString startRatesFile, TString stopRatesFile){
    ...
    TCanvas *canvRateVsTimeFEERIC=new TCanvas("Rate_vs_time","Rate_vs_time");
    TGraph *darkRateVsTimeFEERIC=new TGraph();
    darkRateVsTimeFEERIC->SetName("Dark rate vs time");
    darkRateVsTimeFEERIC->SetTitle("Dark rate vs time");
    darkRateVsTimeFEERIC->SetLineColor(kPink);
    darkRateVsTimeFEERIC->SetMarkerColor(kPink);
    darkRateVsTimeFEERIC->SetMarkerStyle(20);
    darkRateVsTimeFEERIC->SetMarkerSize(0.27);
    ...
    Double_t beginningDarkRates2[nSides][nPlanes][nRPC];
    Double_t endDarkRates[nSides][nPlanes][nRPC];
	Double_t deltaDarkRates[nSides][nPlanes][nRPC];
    Double_t deltaDarkRates2[nSides][nPlanes][nRPC];

    Int_t scalersSide=-1;
    Int_t scalersMt=-1;
    Int_t scalersRpc=-1;
    Double_t scalersValue=-1.;

    ifstream fin2;
    fin2.open(startRatesFile.Data());
    char line2[256];
    while(!fin2.eof()){
        if(fin2.eof())break;
        fin2.getline(line2,256);
        sscanf(line2,"%d %d %d %lf",&scalersMt,&scalersSide,&scalersRpc,&scalersValue);
        beginningDarkRates2[scalersSide][scalersMt][scalersRpc-1]=scalersValue;
        //printf("%d %d %d %lf\n",scalersMt,scalersSide,scalersRpc,scalersValue);
    }
    Printf("\t\t Parsing ended.\n");
    fin2.close();

    ifstream fin3;
    fin3.open(stopRatesFile.Data());
    char line3[256];
    while(!fin3.eof()){
        if(fin3.eof())break;
        fin3.getline(line3,256);
        sscanf(line3,"%d %d %d %lf",&scalersMt,&scalersSide,&scalersRpc,&scalersValue);
        endDarkRates[scalersSide][scalersMt][scalersRpc-1]=scalersValue;
        deltaDarkRates2[scalersSide][scalersMt][scalersRpc-1]=endDarkRates[scalersSide][scalersMt][scalersRpc-1]-beginningDarkRates[scalersSide][scalersMt][scalersRpc-1];
        printf("%d %d %d %lf\n",scalersMt,scalersSide,scalersRpc,deltaDarkRates2[scalersSide][scalersMt][scalersRpc-1]);
    }
    Printf("\t\t Parsing ended.\n");
    fin3.close();
    ...
    for (Int_t side=0; side<nSides; side++) {
        ...
        for (Int_t plane=0; plane<nPlanes; plane++) {
            ...
            for (Int_t RPC=1; RPC<=nRPC; RPC++) {
                ...
                if(beginningDarkRates2[side][plane][RPC-1]!=0.)darkRateVariationVsCharge[side][plane][RPC-1]->SetPoint(0, integratedCharges[side][plane][RPC-1]/Areas[RPC-1][plane], deltaDarkRates2[side][plane][RPC-1]/beginningDarkRates2[side][plane][RPC-1]*100.);
                if(beginningDarkRates2[side][plane][RPC-1]!=0. && beginningDarkCurrents[side][plane][RPC-1]!=0.)darkCurrentVariationVsDarkRateVariation[side][plane][RPC-1]->SetPoint(0, deltaDarkRates2[side][plane][RPC-1]/beginningDarkRates2[side][plane][RPC-1]*100., deltaDarkCurrents[side][plane][RPC-1]/beginningDarkCurrents[side][plane][RPC-1]*100.);
